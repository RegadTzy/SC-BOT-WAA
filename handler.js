...
module.exports = handle = (client, Client) => {
    try {
        ...
        Client.cmd.on('waifu', async (data) => {
            ...
            const buttonMessage = {
			      contentText: 'Waifu',
				  footerText: 'Press the button below to get a random waifu image YNTKTS-BOT',
                        "contextInfo": {
        ...
        Client.cmd.on('setpp', async (data) => {
            ...
            data.reply(`success!, profile picture diubah oleh @${data.sender.split('@')[0]}`)
        })
